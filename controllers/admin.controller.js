...
const getSubAdminClubs = async subAdminId => {
  return await db.clubs.findAll({
    where: { created_by: subAdminId, is_active: true },
    // include: [
    //   {
    //     model: Club,
    //     as: "club",
...
const getUsers = async (req, res) => {
  try {
    ...
    if (currentUser.role === 'super_admin') {
      ...
      const subAdminClubs = await getSubAdminClubs(req.userId)
      const managedClubIds = subAdminClubs.map(cm => cm.id)
      console.log("Club Id's = ", managedClubIds, 'subadminClub = ', subAdminClubs)
